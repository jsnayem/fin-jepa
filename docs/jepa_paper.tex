...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[C]{\small \textit{Preprint. Under review.}}
\fancyfoot[C]{\thepage}
...
\author{\textbf{Yihan Wang}\thanks{First author. \texttt{yihan.wang@example.com}} \\[4pt]
...
To prevent representational collapse, we apply SIGReg~\citep{balestriero2025lejepa}, which projects latent vectors onto random directions and matches their distribution to a standard Gaussian. The regularization randomly projects the $D$-dimensional latents onto 512 random directions using a normalized random matrix, then computes a kernel-based statistic comparing the empirical characteristic function to that of $\mathcal{N}(0,1)$. The SIGReg loss is added to the prediction loss with weight $\lambda = 0.09$.
...
which is computed by projecting latents onto random directions and matching the empirical characteristic function to that of a standard Gaussian~\citep{balestriero2025lejepa}. The number of random projections is set to 512.
...
with $\lambda = 0.09$. This weighting ensures that prediction fidelity dominates the optimization while the regularization term provides a soft constraint against collapse.
...
We conduct comprehensive ablation experiments to identify the optimal architecture configuration. All ablations are trained for 50 epochs on the HS300 subset, using the same training protocol. The primary metric is \textbf{best validation loss}.
...
\textbf{Architecture ablation (Section~4.2):} Fully reproducible. The \texttt{compare\_arch.py} script trains 15 architecture variants on HS300 index constituent stocks using publicly available market data (Yahoo Finance). The script and model code are available in the accompanying code repository. Minor numerical variation may occur due to stochastic hardware and random seeds.

\textbf{Full-scale training (Section~4.3):} Partially reproducible. Training on 6,230 stocks requires a proprietary feature dataset derived from daily OHLCV data. The training script (\texttt{submit\_hf\_job.py}), model definition (\texttt{model.py}), training configuration (hyperparameters, optimizer, scheduler), and evaluation pipeline are included in the repository. The trained model checkpoint and validation loss curve are preserved. External researchers with access to comparable stock data should be able to reproduce the qualitative findings.

\textbf{Downstream evaluation (Section~4.4):} Partially reproducible. The \texttt{experiment\_e.py} script is included in the repository and can be run with any trained JEPA checkpoint.
...
\midrule
Model architecture (\texttt{model.py}) & Repository & $\checkmark$ Open \\
Ablation training script (\texttt{compare\_arch.py}) & Repository & $\checkmark$ Open \\
Full training script (\texttt{submit\_hf\_job.py}) & Repository & $\checkmark$ Open \\
Downstream evaluation script (\texttt{experiment\_e.py}) & Repository & $\checkmark$ Open \\
Ablation results (\texttt{output/arch\_*/meta.json}) & Repository & $\checkmark$ Open \\
Full training checkpoint \& logs & Hugging Face & $\blacksquare$ Private \\
Full training feature dataset & Hugging Face & $\blacksquare$ Private \\
\bottomrule
\end{tabular}
\end{center}

Code is available at: \url{https://github.com/cedricwyh/fin-jepa} (upon request for the active development repository).
