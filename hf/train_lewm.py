"""LeWM-style training for Fin-JEPA on HF Jobs.
Self-contained: includes all model code inline.

Uses LeWM hyperparameters with improvements from keon/jepa and H-JEPA:
- LR=5e-4, AdamW with param_groups
- SIGReg: num_proj=1024, lambda=0.09
- Target LayerNorm (vjepa2): F.layer_norm on targets
- SmoothL1 loss (ijepa): robust to outliers
- LinearWarmupCosineAnnealingLR (1% warmup)
- bf16 precision
- context=3, predict=1 (LeWM style)
- Proper chronological per-stock train/val split (cedwyh/fin-jepa-h3)
"""
# /// script
# dependencies = [
#   "torch>=2.0",
...
def run_exp(config):
    print(f"\n{'='*50}", flush=True)
    print(f"[{config['tag']}] D={config['D']} L={config['L']} lr={config['lr']}", flush=True)
    ...
    result = {'tag':config['tag'],'D':config['D'],'L':config['L'],
              'params':n_params,'best_va':best_loss,'epochs':config['epochs'],
...
VARIANTS = [
    # Winner from sweep: D=192, L=6, LR=5e-4 — now 200ep with proper split + improvements
    {"tag":"lewm_v2", "D":192, "L":6, "lr":5e-4, "nproj":1024, "lam":0.09, "epochs":200, "bs":256,
     "target_ln":True, "smooth_l1":True},
    # Ablation: target_ln=False to test its contribution
    {"tag":"lewm_v2_notln", "D":192, "L":6, "lr":5e-4, "nproj":1024, "lam":0.09, "epochs":100, "bs":256,
     "target_ln":False, "smooth_l1":True},
    # Ablation: smooth_l1=False (use MSE)
    {"tag":"lewm_v2_nosl1", "D":192, "L":6, "lr":5e-4, "nproj":1024, "lam":0.09, "epochs":100, "bs":256,
     "target_ln":True, "smooth_l1":False},
    # Ablation: no weight decay (wd=0)
    {"tag":"lewm_v2_nowd", "D":192, "L":6, "lr":5e-4, "nproj":1024, "lam":0.09, "epochs":100, "bs":256,
     "target_ln":True, "smooth_l1":True},
    # LR=5e-3 ablation — might surprise us
    {"tag":"lewm_v2_lr5e-3", "D":192, "L":6, "lr":5e-3, "nproj":1024, "lam":0.09, "epochs":50, "bs":256,
     "target_ln":True, "smooth_l1":True},
    # LR=5e-3 full — 200ep to match v2 baseline
    {"tag":"lewm_v2_lr5e-3_full", "D":192, "L":6, "lr":5e-3, "nproj":1024, "lam":0.09, "epochs":200, "bs":256,
     "target_ln":True, "smooth_l1":True},
    # LR=5e-2 — aggressive, monitor closely, cancel if diverges
    {"tag":"lewm_v2_lr5e-2", "D":192, "L":6, "lr":5e-2, "nproj":1024, "lam":0.09, "epochs":100, "bs":256,
     "target_ln":True, "smooth_l1":True},
    # ── Fix predictor collapse experiments ──
    # B1: weak predictor (depth=2), forces predictor to rely on context
    {"tag":"lewm_v2_b1", "D":192, "L":2, "lr":5e-4, "nproj":1024, "lam":0.09, "epochs":100, "bs":256,
     "target_ln":True, "smooth_l1":True, "hist":3},
    # B2: longer context (history=8), more info to work with
    {"tag":"lewm_v2_b2", "D":192, "L":6, "lr":5e-4, "nproj":1024, "lam":0.09, "epochs":100, "bs":256,
     "target_ln":True, "smooth_l1":True, "hist":8},
    # B3: no self-conditioning, removes the "cheating" path
    {"tag":"lewm_v2_b3", "D":192, "L":6, "lr":5e-4, "nproj":1024, "lam":0.09, "epochs":100, "bs":256,
     "target_ln":True, "smooth_l1":True, "hist":3, "no_self_cond":True},
    # B4: weak predictor + long context, combined approach
    {"tag":"lewm_v2_b4", "D":192, "L":2, "lr":5e-4, "nproj":1024, "lam":0.09, "epochs":100, "bs":256,
     "target_ln":True, "smooth_l1":True, "hist":8},
]
...
def main():
    ...
    parser.add_argument("--tag", default="lewm-v2-sweep")
    parser.add_argument("--variants", type=int, default=0)
    parser.add_argument("--start", type=int, default=0)
    args = parser.parse_args()

    print("═"*60, flush=True)
    print(f"LeWM v2 Fin-JEPA (proper chronological split) | {DEVICE}", flush=True)
    print(f"Tag: {args.tag} | variants: {args.variants or len(VARIANTS)}", flush=True)
    ...
    for r in results:
        print(f"  {r['tag']:20s} | D={r['D']:3d} L={r['L']} | params={r['params']:>7,} | "
              f"va={r['best_va']:.4f} | {r['elapsed']:.0f}s | "
              f"tln={r.get('target_ln','?'):5} sl1={r.get('smooth_l1','?'):5}", flush=True)
    ...
    try:
        upload_folder(folder_path=str(BASE), repo_id=DATA_REPO, repo_type="dataset",
                      commit_message=f"LeWM v2 sweep {args.tag}: {len(results)} variants",
